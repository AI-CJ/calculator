...
function saveHistory() { localStorage.setItem(STORE.HISTORY, JSON.stringify(state.history.slice(-50))); }
...
themeToggle.addEventListener('click', () => {
  ...
  try { localStorage.setItem(STORE.THEME, light ? 'light' : 'dark'); } catch {}
});
...
degToggle.addEventListener('click', () => {
  ...
  degToggle.textContent = state.deg ? 'DEG' : 'RAD';
  degToggle.setAttribute('aria-pressed', state.deg ? 'true' : 'false');
  try { localStorage.setItem(STORE.DEG, String(state.deg)); } catch {}
...
document.addEventListener('keydown', (e) => {
  ...
  // Function/const shorthands
  else if (k.toLowerCase() === 'p') val = 'pi';
  else if (/^[a-zA-Z]$/.test(k)) val = k.toLowerCase(); // allow typing "sin", "ln", etc.

  if (val !== null) {
    handleInput(val);
    const selector = (() => {
      try {
        const escaped = (window.CSS && CSS.escape) ? CSS.escape(val) : String(val).replace(/"/g, '\\"');
        return `button[data-value="${escaped}"]`;
      } catch { return null; }
    })();
    if (selector) {
...
function handleInput(value) {
  // Start fresh if result is showing and user starts a new number/func/paren/dot
  ...
  if (value === 'pi') { state.expr += 'pi'; setDisplay(prettyExpr(state.expr)); return; }
  if (value === 'ANS') { state.expr += 'ANS'; setDisplay(prettyExpr(state.expr)); return; }
  ...
  if (value === '=') {
    ...
    try { localStorage.setItem(STORE.LAST, state.lastResult); } catch {}
...
function evaluateExpressionSafe(expr, deg, ansStr) {
  try {
    ...
    return Number(Number(res).toFixed(12)); // trim noise
  } catch {
    return 'Err';
  }
}
...
function tokenize(expr) {
  ...
  while (i < expr.length) {
    ...
    // Pretty operators and constants
  ...
  // Process unary minus and postfix percent
  const out = [];
  for (let j = 0; j < tokens.length; j++) {
    const t = tokens[j];
    const prev = out[out.length - 1];

    // Unary minus if at start or follows operator or '('
    if (t.type === 'operator' && t.value === '-') {
      if (!prev || (prev.type === 'operator' || prev.type === 'lparen')) {
        out.push({ type: 'operator', value: 'u-' });
        continue;
      }
    }

    // Postfix percent: convert '%' to 'u%' when it follows an operand or ')'
    if (t.type === 'operator' && t.value === '%') {
      if (prev && (prev.type === 'number' || prev.type === 'const' || prev.type === 'rparen')) {
        out.push({ type: 'operator', value: 'u%' });
        continue;
      }
    }
...
function toRPN(tokens) {
  ...
  const prec = (op) => {
    switch (op) {
      case 'u-':
      case 'u%': return 5; // unary operators
      case '^': return 4;
      case '*':
      case '/': return 3;
      case '+':
  ...
  const rightAssoc = (op) => op === '^' || op === 'u-' || op === 'u%';
  ...
  for (const t of rpn) {
    ...
    if (t.type === 'operator') {
      ...
      switch (t.value) {
        ...
        case '*': stack.push(a * b); break;
        case '/':
          if (b === 0) return Infinity; // mapped to Err by caller
          stack.push(a / b);
          break;
    ...
    if (t.type === 'func') {
      const a = stack.pop(); if (a === undefined) throw new Error('Stack underflow');
      switch (t.value) {
        case 'sin': stack.push(Math.sin(state.deg ? toRad(a) : a)); break;
        case 'cos': stack.push(Math.cos(state.deg ? toRad(a) : a)); break;
        case 'tan': {
          const angle = state.deg ? toRad(a) : a;
        ...
        case 'sqrt':
          ...
          break;
        default: throw new Error('Func ' + t.value);
      }
      continue;
    }
